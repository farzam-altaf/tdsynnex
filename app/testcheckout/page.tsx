...
export default function Page() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // STEP 1: Check stock for all products before processing
            const stockChecks = await Promise.all(
                cartItems.map(async item => {
                    const product = item.product;
                    if (!product) throw new Error(`Product details not found for ${item.product_id}`);

                    const currentStock = product.stock_quantity ?? "0"; // fallback in case undefined
                    const requestedQty = item.quantity ?? 0;           // fallback

                    const intCurrStock = parseInt(String(currentStock), 10); // always string
                    const intRequestedQty = parseInt(String(requestedQty), 10);


                    if (requestedQty > currentStock) {
                        throw new Error(`Insufficient stock for ${product.product_name}. Available: ${currentStock}, Requested: ${requestedQty}`);
                    }
                    ...
                    return {
                        ...
                        oldWithCustomer: Number(product.withCustomer || 0),
                        newWithCustomer: Number(product.withCustomer || 0) + requestedQty
                    };
                })
            );
            ...
            // STEP 3: Update all product stocks in Supabase
            for (const check of stockChecks) {
                const { product, newStock, newWithCustomer } = check;
                ...
                // STEP 4: Update WooCommerce stock by SKU
                await fetch('/api/woocommerce/update-stock', {
                    method: 'POST',
            ...
            // STEP 7: Redirect
            setTimeout(() => router.push('/thanks'), 50);
