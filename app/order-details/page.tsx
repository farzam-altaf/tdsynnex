...
export default function Page() {
    ...
    try {
      ...
      await logActivity({
        ...
        details: {
          ...
          executionTimeMs: Date.now() - startTime,
          userRole: profile?.role,
          updatedFields: Object.keys(selectedOrder).filter(
            (key) =>
              selectedOrder[key as keyof Order] !==
  ...
  const columns: ColumnDef<Order>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const amount = row.getValue("rev_opportunity") as number | null;
        return <div className="text-left ps-2">{formatCurrency(amount)}</div>;
      },
    },
    {
      accessorKey: "dev_budget",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => {
        const amount = row.getValue("dev_budget") as number | null;
        return <div className="text-left ps-2">{formatCurrency(amount)}</div>;
      },
    },
    {
      accessorKey: "dev_opportunity",
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => {
        const amount = row.getValue("dev_opportunity") as number | null;
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-left ps-2 lowercase">
          {row.getValue("se_email") || "-"}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-left ps-2">{row.getValue("vertical") || "-"}</div>
      ),
    },
    {
      accessorKey: "segment",
      ...
      cell: ({ row }) => (
        <div className="text-left ps-2">{row.getValue("segment") || "-"}</div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-left ps-2">
          {row.getValue("order_quarter") || "-"}
        </div>
      ),
    },
  ];

  const isAdminOrManager = profile?.role === adminRole || profile?.role === smRole;
      ...
      cell: ({ row }) => {
        ...
        const handleConfirmDelete = async () => {
          ...
          try {
            ...
            if (
              !nonReturnableStatuses.includes(orderData.order_status) &&
              orderData.products &&
              orderData.quantity
            ) {
              ...
              if (orderQuantity > 0) {
                ...
                const currentStockQty =
                  parseInt(orderData.products.stock_quantity) || 0;
                const currentWithCustomerQty =
                  parseInt(orderData.products.withCustomer) || 0;

                // Calculate new quantities
                const newStockQty = currentStockQty + orderQuantity; // Increase stock
                const newWithCustomerQty = Math.max(
                  0,
                  currentWithCustomerQty - orderQuantity,
                ); // Decrease withCustomer

                // Ensure quantities don't go negative
                const finalStockQty = Math.max(0, newStockQty).toString();
                const finalWithCustomerQty = Math.max(
                  0,
                  newWithCustomerQty,
                ).toString();
            ...
            await logActivity({
              ...
              details: {
                ...
                stockUpdated: !nonReturnableStatuses.includes(
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  className="cursor-pointer"
                  onClick={() =>
                    navigator.clipboard.writeText(order.company_name || "")
                  }
                >
                  Copy Customer Name
                </DropdownMenuItem>
        ...
        title={
          <div>
            <div className="text-xl font-bold text-gray-800">
              Edit Order #
              <span className="text-[#0A4647]">{selectedOrder?.order_no}</span>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <Select.Option value="Customer needs to use it in their own environment">
                      Customer Environment
                    </Select.Option>
                    <Select.Option value="Distributor needs it for their own conferences/events">
                      Distributor Events
                    </Select.Option>
                    <Select.Option value="Reseller looking to use it for one of their events">
                      Reseller Events
                    </Select.Option>
                  </Select>
                  {editErrors.use_case && (
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      handleSelectChange("currently_running", value)
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      handleSelectChange("isSecurity", value)
                    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <Select.Option value="District Of Columbia">
                    ...
                    <Select.Option value="Armed Forces (AA)">
                      Armed Forces (AA)
                    </Select.Option>
                    <Select.Option value="Armed Forces (AE)">
                      Armed Forces (AE)
                    </Select.Option>
                    <Select.Option value="Armed Forces (AP)">
                      Armed Forces (AP)
                    </Select.Option>
